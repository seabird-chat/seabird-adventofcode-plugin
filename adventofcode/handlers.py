...
class AdventOfCodeClient:
    ...
    async def handle_command(self, command):
        if command.command == 'aoc':
            ...
            await self.seabird.send_message(channel_id=command.source.channel_id, text=', '.join(scores))

    async def lookup_leaderboard(self, event=None):
        if not event:
            event = str(datetime.today().year)

        async with self.aoc.get(f'https://adventofcode.com/{event}/leaderboard/private/view/{self.leaderboard_id}.json') as response:
